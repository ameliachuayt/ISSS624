---
title: "In-class Ex4: Calibrating Hedonic Pricing Model for Private Highrise Property with Geographically Weighted Regression"
author: "Amelia Chua"
...
After transforming the projection metadata, you can verify the projection of the newly transformed *`mpsz_svy21`* by using `st_crs()` of **sf** package. Although the original data set is in SVY21, we can transforming it to be in EPSG 3414 which is an international code.
...
The next code chunk builds the map. Note that we included "tmap_options(check.and.fix = TRUE)" to bypass some existing error with the mpsz_svy21 file.
...
The code chunk below is used to plot a scatterplot matrix of the relationship between the independent variables in `condo_resale` data.frame. We are using the original data frame as using the `condo_resale.sf` will return an error due to it containing a geometry column.
...
We will use *ols_regress()* from the **olsrr** package to print out a publication quality table. This allows us to easy view model summary statistics.
...
We can see that the Adj R-Square is 0.647. We are using the adjusted value to penalise the model as it includes more than 1 explanatory variable.

Looking at the coefficients, we can interpret both the value and the sign. For instance, if the coefficient is a negative value, we can infer that there is an inverse relationship between the explanatory variable and dependent variable. For instance, there is an inverse relationship (between distance to MRT and price. This means that the further away from the MRT station, the lower the price of the condo.

An area for further investigation is the coefficient for proximity to primary school. Since the coefficient is positive, it means that the further away from the school, the more expensive the condo. This may be counter-intuitive as parents would want schools nearer to home. On the other hand, living near primary schools may also have disadvantages like noise (school alarm bells).
...
Next, we will join the newly created data frame of the residuals with `condo_resale.sf` object.
...
In the code chunk below *bw.gwr()* of **GWModel** package is used to determine the optimal fixed bandwidth to use in the model. Notice that the argument ***adaptive*** is set to **FALSE** indicates that we are interested to compute the **fixed bandwidth**. Note that when we indicate longlat = FALSE, our input data is already in the project coordinate system and no conversion is required.
...
The CV score is to be minimised. The result shows that the recommended bandwidth is 971.3405 metres.
...
The report shows that the adjusted r-square of the gwr is 0.8430 which is significantly better than the global multiple linear regression model of 0.6472. Similarly, we can also compare the AICc values.
...
The CV score is to be minimised. The result shows that the 30 is the recommended data points to be used.
...
The code chunks below is used to create an interactive point symbol map. The zoom limits basically limits the zoom limits, beyond which, the maps cannot be further zoomed in or out.
...
The code chunks below is used to create an interactive point symbol map. By including 'sync = TRUE' both maps will synchronise together with cursor movements which allows for easy viewing and better user experience.

The two maps plotted are the observed values and the t-statistics.
...
#### Visualising Coefficient Estimates by Planning Area
...
Tin Seong Kam. "6 Calibrating Hedonic Pricing Model for Private Highrise Property with GWR Method" From **R for Geospatial Data Science and Analytics** <https://r4gdsa.netlify.app/chap04.html>
