...
Additionally, we can also customise the choropleth to show exactly which are predicted functional and non-functional water points. To do this, we first extract out the functional and non-functional water points based on the threshold / cut-off value of 0.5 into two data frames using the code chunk below.
...
Then, we can re-plot the choropleth using the newly derived data frames. Here, we are in a sense, overlaying layers of maps on-top of one another.
...
In the figure above, green coloured dots refers to functional water points and orange coloured dots refers to non-functional water points.
...
The code chunk below will plot the choropleth maps with the threshold levels set as 0.50 (left map) and 0.30 (right map). The maps are arranged using *tmap_arrange()* where we included `sync = TRUE` so that both maps will synchronise together with cursor movements which allows for easy viewing and better user experience.
...
Let's also plot the choropleth maps with the threshold levels set as 0.50 (left map) and 0.70 (right map). To mix things up, note that we are using *tm_bubbles()* instead of *tm_dots()* here. As this as a non-interactive map, using *tm_bubbles()* gives a better visual effect than *tm_dots()*. The maps are arranged using *tmap_arrange().*
