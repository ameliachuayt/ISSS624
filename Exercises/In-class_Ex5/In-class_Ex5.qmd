...
Then, we can re-plot the choropleth of the predicted values using the newly derived data frames. We are also plotting the choropleth for the observed values. We arrange the maps using *tmap_arrange()*, with the actual/observed values on the left and the predicted values on the right.
